...
public class MainPanel extends JPanel {
	...
	//Part 1, Task H2: Implement RBAC for the JFM tool
	if (rpm != null) {
		for (int i = 0; i < rpm.length; i++) {
			System.out.println("Privileges for " + userRole + " role: " + rpm[i] + i);

			if (rpm[i].equals("View")) {
				buttons.addElement(f3Button);
			} else if (rpm[i].equals("Edit")) {
				buttons.addElement(f4Button);
			} else if (rpm[i].equals("Copy")) {
				buttons.addElement(f5Button);
			} else if (rpm[i].equals("Move")) {
				buttons.addElement(f6Button);
			} else if (rpm[i].equals("Mkdir")) {
				buttons.addElement(f7Button);
			} else if (rpm[i].equals("Quit")) {
				buttons.addElement(f10Button);
			} else if (rpm[i].equals("Logout")) {
				buttons.addElement(logoutButton);
			} else if (rpm[i].equals("AddUser")) {
				buttons.addElement(addNewUser);
			} else if (rpm[i].equals("ChangePassword")) { //Part 1, Task 3: Adding changePassword button
